...
|------|---------|--------------|---------|--------|
| index.html | 1.3 | 2026-05-22 | All game screens. v1.3: added `screen-host-landing` (host game manager) + `host-persistent-banner` (room-code banner pinned to dashboard). v1.2: `screen-gym-review`. v1.1 (1.6): `screen-test-build-complete`. | Active |
...
| game.js | 1.8 | 2026-05-22 | Game logic. v1.8: room-code rejoin (returning player's localStorage ID matches `room.players` → restore save, never overwrite) + 15-second `last_seen` presence heartbeat + host game manager (active/archived lists, recency sort, archive flag write-through, auto-archive on GAME_OVER, persistent room-code banner, presence column derived from heartbeat). v1.7: completed-gym Review Mode. v1.6.1: seen-question-set draw-without-replacement + runtime unscramble shuffle. | Active |
...
| FILES.md | 1.12 | 2026-05-22 | This manifest | Active |
