...
|------|---------|--------------|---------|--------|
| index.html | 1.8.3 | 2026-05-25 | All game screens. v1.8.3: screen-battle shell added (Commit 1 scaffold). v1.8.2: peso removed from player UI; pre-game pokeball copy "3 Pokeball(s)" → "1 Pokeball". v1.8.1: mascot alt text + Region 10 Game Complete screen. v1.8: player dashboard three-column rewrite. v1.7.2: Room Detail Overlay Start Game + drop Copy Code. v1.7.1: lobby cap label 0/8 → 0/5. v1.7: host dashboard three-column. v1.6: persistent identity. v1.5: wallet + host crystal panels. v1.4: removed "Continue Journey". v1.3: host-landing. v1.2: gym-review. v1.1: test-build lid. | Active |
| style.css | 1.7.3 | 2026-05-25 | Styling. v1.7.3: battle screen CSS scaffold (Commit 1) — `.battle-header`, `.battle-hp-section` + `.battle-hp-bar` (with `.enraged` variant), `.battle-contrib-row` + `.battle-pip`, `.battle-controls`, `.battle-round-result`, `.battle-win` + `.battle-reward-offer`, `.battle-loss`, `.darkrai-cameo-overlay`. v1.7.2: team-cap UI for Part 12E. v1.7.1: dead `.region-card.coming-soon` removed. v1.7: player three-column dashboard classes. v1.6.1: `.wl-slots-grid` repeat(5,1fr). v1.6: three-column host dashboard. v1.5: persistent-identity surfaces. v1.4: wallet + host crystal panels. v1.3: host-landing + presence. v1.2: gym-review. v1.1: `.region-card.coming-soon`. | Active |
| game.js | 1.18 | 2026-05-25 | Game logic. v1.18: Battle engine scaffold — BOSS_DATA + BOSS_REWARD_POKEMON constants; BOSS_FIGHT phase wired into hostNextPhase/col3/reconnect; startBossFight() stub (Commit 2 replaces body). ⚠️ Boss-crystal mismatch confirmed clean — no boss crystal logic existed in v0.6.0, nothing to strip. v1.17: economy engine v3.3 LIVE (SPEC Part 12). | Active |
...
| FILES.md | 1.33 | 2026-05-25 | This manifest. v1.33: battle engine Commit 1 scaffold — game.js v1.18, index.html v1.8.3, style.css v1.7.3. v1.32: SPEC v3.7, pokemon.json v2.2, BACKLOG.md v1.1, CLAUDE.md sync note. | Active |
